...
let globalEmployeeCache: {
    products: InventoryProduct[];
    logs: ActivityLog[];
} | null = null;

export default function EmployeeDashboard() {
    const [products, setProducts] = useState<InventoryProduct[]>(globalEmployeeCache?.products || []);
    const [logs, setLogs] = useState<ActivityLog[]>(globalEmployeeCache?.logs || []);
    const [loading, setLoading] = useState(globalEmployeeCache === null);
    ...
    const fetchProfile = useCallback(async () => {
        ...
        setDisplayName(result?.data?.displayName || "there");
        setFirstName(result?.data?.firstName ?? "");
    }, []);

    /**
     * Fetches dashboard data including products and recent activity logs from the backend.
     * Interacts with /api/admin/fetch-products and /api/admin/fetch-logs endpoints.
     * @param isBackground - If true, avoids triggering the visible loading state (used for silent polling).
     */
    const fetchDashboardData = useCallback(async (isBackground = false) => {
        try {
            if (!isBackground && !globalEmployeeCache) {
                setLoading(true);
            }
            setError(null);
            ...
            globalEmployeeCache = { products: fetchedProducts, logs: fetchedLogs };
    ...
    return (
        <div className="space-y-6">
            <div>
                <p className="text-xs uppercase tracking-[0.2em] text-gray-400">Employee Home</p>
                <h1 className="text-3xl font-bold text-[#121212] mt-1">Good day, {firstName || "Employee"}</h1>
